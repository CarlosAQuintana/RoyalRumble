...
public class vertIcicleSpikeScript : MonoBehaviour
{
    Rigidbody rb;
    ...
    void Update()
    {
        ...
        if (hM.vertIceLaneSet == 1)
        {
            rb.velocity = new Vector3(0, 0, 100);
        }
        if (hM.vertIceLaneSet == 2)
        {
            rb.velocity = new Vector3(0, 0, -100);
        }
        if (hM.vertIceLaneSet == 3)
        {
            rb.velocity = new Vector3(0, 0, 100);
        }
        if (hM.vertIceLaneSet == 4)
        {
            rb.velocity = new Vector3(0, 0, -100);
        }
        if (hM.vertIceLaneSet == 5)
        {
            rb.velocity = new Vector3(0, 0, 100);
        }
